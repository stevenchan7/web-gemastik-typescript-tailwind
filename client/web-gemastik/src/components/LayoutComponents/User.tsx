...
export default function User() {
  ...
  return(
    ...
    {/* User offcanvas menu  */}
    <div className={`flex justify-end item-center ${ isOpen ? 'w-screen' : 'w-0' } overflow-hidden fixed top-0 right-0 h-screen bg-slate-700/25 z-10`}>
      ...
      <div className={`flex flex-col justify-start item-start p-4 gap-4 text-base ${ isOpen ? 'w-60' : 'w-0' } h-screen overflow-hidden bg-slate-500 transition-[width] duration-500`}>
        <div className="w-fit rounded-full overflow-hidden cursor-pointer" onClick={() => setIsOpen(false)}>
          <Image src='/Layout/default-user.png' width={40} height={40} alt='User Photo' />
        </div>
