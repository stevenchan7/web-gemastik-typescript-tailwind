...
export default function EventDialog({ eventData }: EventDialogProps) {
	...
	return (
		<AlertDialog.Root>
			...
			<AlertDialog.Portal>
				<AlertDialog.Overlay className='bg-primary-text-black/70 data-[state=open]:animate-overlayShow fixed inset-0' />
				<AlertDialog.Content className='data-[state=open]:animate-contentShow overflow-y-auto fixed top-[50%] left-[50%] max-h-[85vh] w-[90vw] max-w-[500px] translate-x-[-50%] translate-y-[-50%] rounded-[6px] bg-primary-bg p-[25px] shadow-[hsl(206_22%_7%_/_35%)_0px_10px_38px_-10px,_hsl(206_22%_7%_/_20%)_0px_10px_20px_-15px] focus:outline-none'>
					<AlertDialog.Title className='m-0 text-[17px] font-medium'>
						...
						<div>
							<h3 className='font-semibold text-primary-text-green'>{eventData.community}</h3>
						</div>
					</AlertDialog.Title>
					...
					<div className='flex justify-end gap-[25px]'>
						<AlertDialog.Cancel asChild>
							<button className='bg-primary-text-black/20 hover:bg-primary-text-black/50 focus:shadow inline-flex h-[35px] items-center justify-center rounded-[4px] px-[15px] font-medium leading-none outline-none'>
								Cancel
							</button>
						</AlertDialog.Cancel>
						<AlertDialog.Action asChild>
							<button className='text-text-white bg-primary-text-green hover:bg-primary-text-green/80 hover:scale-105 focus:shadow inline-flex h-[35px] items-center justify-center rounded-[4px] px-[15px] font-medium leading-none outline-none'>
								Yes, delete account
							</button>
						</AlertDialog.Action>
	 				</div>
