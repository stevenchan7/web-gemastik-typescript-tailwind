...
function CardText({ children }: { children: string }) {
	...
	let text =
		'Kegiatan bersih-bersih pantai, mengumpulkan sampah-sampah yang mengotori pantai. Kegiatan ini bertujuan untuk membersihkan ekosistem pantai yang tercemari sampah hasil kegiatan atau aktivitas manusia di pantai';
	return (
		<>
			<p className='text-sm mb-2'>{isShowMore ? children : children.slice(0, 120) + '...'}</p>
			<span className='text-sm font-bold cursor-pointer' onClick={() => setIsShowMore(!isShowMore)}>
...
export default function ActivityTypeCard({ title, desc }: { title: string; desc: string }) {
	return (
		<div className='w-[300px] min-h-[350px] flex flex-wrap m-auto bg-primary-bg border border-[#D6925C] rounded-xl'>
			...
			<div className='mx-4 mb-4 text-primary-black'>
